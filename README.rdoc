...
Robut::Plugin::Base and implement the <tt>handles?(time, sender_nick,
message)</tt> and <tt>handle(time, sender_nick, message)</tt>
methods. <tt>handles?</tt> returns true if the plugin cares about the
message, false otherwise. +handle+ performs any plugin-specific logic.
